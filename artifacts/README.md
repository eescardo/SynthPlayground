...
The workflow uploads an `after` artifact bundle and leaves a PR comment with embedded poster images linking to hosted GitHub Pages player pages, plus download links.

Current implementation detail:

- Hosted PR video preview files are published to a dedicated utility branch named `pr-video-previews`.
- The workflow keeps the captured `.webm` files in the artifact bundle and publishes per-PR player pages through GitHub Pages for browser playback from the PR comment.
- That branch is reusable scratch space for generated preview assets and may grow over time.
- If the branch is deleted, the workflow will recreate it on the next run.
