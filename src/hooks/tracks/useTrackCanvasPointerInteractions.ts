...
export function useTrackCanvasPointerInteractions({
  ...
  const [hoveredCompositionEnd, setHoveredCompositionEnd] = useState(false);
  ...
  const findNoteRect = useCallback(
    (x: number, y: number): NoteRect | null => {
  ...
  const getLaneAtPoint = useCallback(
    (x: number, y: number): { track: Track; lane: AutomationLaneLayout } | null => {
      if (x < headerWidth) {
        return null;
      }
      const layout = getTrackLayoutAtY(y);
  ...
  const resolvePointerTargets = useCallback(
    (x: number, y: number) => {
  ...
  const onPointerMove = useCallback(
    (event: React.PointerEvent<HTMLCanvasElement>) => {
      ...
      setHoveredLoopMarker((prev) => {
        const next =
          !targets.laneHit && targets.hoverTarget === "loop-marker" && targets.loopMarkerRect
            ...
            : null;
        return prev?.markerId === next?.markerId && prev?.kind === next?.kind && prev?.beat === next?.beat
          ? prev
          : next;
      });
      setHoveredPitch((prev) => {
        const next = targets.pitchRect
          ? { trackId: targets.pitchRect.trackId, noteId: targets.pitchRect.noteId }
          : null;
        ...
        const fixedLaneHit =
          x >= headerWidth
            ? getTrackLayoutAtY(y)?.automationLanes.find(
                (entry) => !entry.automated && y >= entry.y && y <= entry.y + entry.height
              )
            : null;
      ...
      if (drag.mode === "move") {
        noteActions.onUpdateNote(
          drag.trackId,
          drag.noteId,
          { startBeat: Math.max(0, snapToGrid(beat - drag.offsetBeats, gridBeats)) },
          {
    ...
    (event: React.PointerEvent<HTMLCanvasElement>) => {
      ...
      if (pendingLaneAction?.kind === "automation-keyframe") {
        ...
        if (lane) {
          ...
          automationActions.onPreviewTrackMacroAutomation(automationDrag.trackId, automationDrag.macroId, finalValue, {
            beat: automationDrag.beat,
            retrigger: true
          });
        }
  ...
  const onPointerLeave = useCallback(
    (event: React.PointerEvent<HTMLCanvasElement>) => {
      ...
      setHoveredCompositionEnd(false);
