...
export function useTrackMacroAutomationActions({
  ...
  const applyPreviewState = useCallback(
    (trackId: string, macroId: string, normalized: number, beat?: number) => {
  ...
  const previewTrackMacroAutomation = useCallback(
    (trackId: string, macroId: string, normalized: number, options?: { retrigger?: boolean; beat?: number }) => {
  ...
  const bindTrackMacroToAutomation = useCallback(
  ...
  const unbindTrackMacroFromAutomation = useCallback(
  ...
  const toggleTrackMacroAutomationLane = useCallback(
    (trackId: string, macroId: string) => {
      commitProjectChange(
        (current) => ({
          ...current,
  ...
  const upsertTrackMacroAutomationKeyframe = useCallback(
    (
      ...
      commitTrackMacroAutomationLaneChange(
        ...
        macroId,
        (lane, current) =>
          upsertAutomationLaneKeyframe(lane, beat, value, getProjectTimelineEndBeat(current), options?.keyframeId),
  ...
  const splitTrackMacroAutomationKeyframe = useCallback(
    (trackId: string, macroId: string, keyframeId: string) => {
      commitTrackMacroAutomationLaneChange(trackId, macroId, (lane) => splitAutomationLaneKeyframe(lane, keyframeId), {
  ...
  const deleteTrackMacroAutomationKeyframeSide = useCallback(
    (trackId: string, macroId: string, keyframeId: string, side: AutomationKeyframeSide) => {
      commitTrackMacroAutomationLaneChange(
        trackId,
        macroId,
