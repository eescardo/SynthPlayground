...
export function useTrackMacroPanelState({ tracks, commitProjectChange }: UseTrackMacroPanelStateArgs) {
  const setTrackMacroPanelExpanded = useCallback(
    (trackId: string, expanded: boolean) => {
      commitProjectChange(
        (current) => {
          let changed = false;
  ...
  const toggleTrackMacroPanel = useCallback(
    (trackId: string) => {
